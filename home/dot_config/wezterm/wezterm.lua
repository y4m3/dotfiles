...
if wezterm.target_triple:find("windows") then
  ...
  -- launcher (and the new-tab button's right-click menu) for crossing
  -- into PowerShell (winget: Microsoft.PowerShell). domain = "local"
...
-- claiming it steals nothing from bash, tmux, or nvim below.
...
-- it, so everything but the tab bar still comes from Tracer above. Values
-- below are quoted straight from colors/tracer.toml; if this grows past
-- the tab bar it should move upstream into tracer-color's build.py as
-- [colors.tab_bar] instead of living here.
config.colors = {
  tab_bar = {
...
-- rounded-corner renderer, capped so a long shell path can't crowd out
-- the other tabs.
